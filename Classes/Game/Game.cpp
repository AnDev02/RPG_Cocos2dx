...
bool Game::init()
{
    ...
    inGameUI = InGameUI::create(_player);
...
void Game::updateUserInterface(float dt) {
    ...
    //_joystick->setPosition(_player->getPosition() - Vec2((visibleSize.width / 2.7 / zoomLevel), visibleSize.height / 2 / zoomLevel - (_joystick->getSize().height / 2)));
}