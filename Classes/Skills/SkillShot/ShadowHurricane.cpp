#include "ShadowHurricane.h"
#include "PlayerCharacters/PlayerCharacter.h"
#include <string>
bool ShadowHurricane::init() {
    ...
    //Skill Icon...
    _iconSprite = Sprite::create("skill/SkillSprite/SkillIcon/27.png");
    _iconSprite->setScale(0.1);
    _iconSprite->retain();

    SpriteFrameCache::getInstance()->addSpriteFramesWithFile("skill/SkillSprite/SkillShot/shadow_hurricane/shadow_hurricane.plist", "skill/SkillSprite/SkillShot/shadow_hurricane/shadow_hurricane.png");
    //Skill Sprite
    _skillSprite = Sprite::createWithSpriteFrameName("shadow_hurricane (1).png");
    _skillSprite->setAnchorPoint(Vec2(0.5, 0.5));
    _skillSprite->retain();

    //Skill Animate
    _skillAnimate = Animate::create(Engine::createAnimation2("shadow_hurricane", 5, 0.05));
    ...
    touchListener->onTouchBegan = CC_CALLBACK_2(ShadowHurricane::onTouchBegan, this);
    touchListener->onTouchMoved = CC_CALLBACK_2(ShadowHurricane::onTouchMoved, this);
    touchListener->onTouchEnded = CC_CALLBACK_2(ShadowHurricane::onTouchEnded, this);
    _eventDispatcher->addEventListenerWithSceneGraphPriority(touchListener, _skillButton);
    schedule(CC_SCHEDULE_SELECTOR(ShadowHurricane::update), 0.05f);

    return true;
}

bool ShadowHurricane::onTouchBegan(Touch* touch, Event* event)
{
...
void ShadowHurricane::onTouchMoved(Touch* touch, Event* event)
{
...
void ShadowHurricane::onTouchEnded(Touch* touch, Event* event)
{
...
void ShadowHurricane::performSkill(Vec2 target) {
