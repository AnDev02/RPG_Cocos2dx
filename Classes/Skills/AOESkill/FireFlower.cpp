...
#include <string>
bool FireFlower::init() {


    skillTalent = new SkillTalent;
    skillTalent->skillName = "FireFlower";
    skillTalent->iconPath = "skill/SkillSprite/SkillIcon/12.png";
    skillTalent->unlockPoint = 10;
    skillTalent->description = "good";
    skillTalent->skillType = "AOE";
    skillTalent->children.push_back(SkillFactory::createSkill("ShadowArea")->getSkillTalent());
    ...
    SpriteFrameCache::getInstance()->addSpriteFramesWithFile("skill/SkillSprite/AOESkill/fire_flower/fire_flower.plist", "skill/SkillSprite/AOESkill/fire_flower/fire_flower.png");
    //Skill Sprite
    _skillSprite = Sprite::createWithSpriteFrameName("fire_flower (1).png");
    _skillSprite->setAnchorPoint(Vec2(0.5, 0.5));
    _skillSprite->retain();

    //Skill Animate
    _skillAnimate = Animate::create(Engine::createAnimation2("fire_flower", 11, 0.05));
    ...
    touchListener->onTouchBegan = CC_CALLBACK_2(FireFlower::onTouchBegan, this);
    touchListener->onTouchMoved = CC_CALLBACK_2(FireFlower::onTouchMoved, this);
    touchListener->onTouchEnded = CC_CALLBACK_2(FireFlower::onTouchEnded, this);
    _eventDispatcher->addEventListenerWithSceneGraphPriority(touchListener, _skillButton);
    schedule(CC_SCHEDULE_SELECTOR(FireFlower::update), 0.05f);

    return true;
}

bool FireFlower::onTouchBegan(Touch* touch, Event* event)
{
...
void FireFlower::onTouchMoved(Touch* touch, Event* event)
{
...
void FireFlower::onTouchEnded(Touch* touch, Event* event)
{
...
void FireFlower::performSkill(Vec2 target) {
